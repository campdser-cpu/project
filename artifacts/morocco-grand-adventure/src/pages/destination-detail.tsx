                ...
                <div className="bg-card border border-border rounded-3xl overflow-hidden shadow-sm flex flex-col md:flex-row">
                  <div className="md:w-1/2 h-64 md:h-auto">
                    {(() => {
                      ...
                      return <img src={`/images/food/${['tagine','couscous','streetfood','pastries','tea'][destination.id.length % 5]}.webp`} alt={`Traditional food in ${destination.name}`} width={800} height={600} loading="lazy" decoding="async" className="w-full h-full object-cover" onError={(e) => { (e.target as HTMLImageElement).src = '/images/food/tagine.webp'; }} />;
                    })()}
                  </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {nearbyDestinations.map(dest => (
              <Link key={dest.id} href={`/destinations/${dest.id}`} className="group relative h-64 rounded-2xl overflow-hidden border border-transparent hover:border-primary/50 transition-all shadow-sm hover:shadow-xl">
                <img src={dest.image} srcSet={`${dest.image.replace(/\.webp$/, '-480w.webp')} 480w, ${dest.image.replace(/\.webp$/, '-768w.webp')} 768w, ${dest.image} 1200w`} sizes="(max-width: 768px) 100vw, (max-width: 1024px) 50vw, 33vw" alt={dest.name} width={1200} height={675} loading="lazy" decoding="async" className="w-full h-full object-cover transition-transform duration-700 group-hover:scale-110" />

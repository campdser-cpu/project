...
function buildEmailHtml(i: Record<string, string>): string {
  const row = (k: string, v: string): string =>
    v ? `<tr><td style="padding:6px 0;font-weight:bold;white-space:nowrap;padding-right:16px;">${escapeHtml(k)}</td><td style="padding:6px 0;">${escapeHtml(v)}</td></tr>` : '';
  return '<div style="font-family:Arial,Helvetica,sans-serif;font-size:14px;color:#222;">' +
    ...
    row('First name', i.firstName) + row('Last name', i.lastName) + row('Email', i.email) + row('Phone', i.phone) +
    row('Travel dates', i.travelDates) + row('Travelers', i.travelers) + row('Destinations', i.destinations) +
